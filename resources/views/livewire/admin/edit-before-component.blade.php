                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <form class="form-horizontal" files="true" enctype="multipart/form-data">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" id="example-fileinput" class="form-control"  wire:model="newbefore"/>
                                     @if($newbefore)
                                            <img src="{{ $newbefore->temporaryUrl() }}" width="120"/>
                                        @else
                                            <img src="{{ asset('assets/img/before')}}/{{ $before }}" width="120"/>
                                        @endif
                                    @error('newbefore')<p class="text-danger">{{ $message }}</p>@enderror
                                </div>
                        </div> <!-- end col -->
                        ...
                        <div class="col-lg-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" id="example-fileinput" class="form-control"  wire:model="newafter"/>
                                      @if($newafter)
                                            <img src="{{ $newafter->temporaryUrl() }}" width="120"/>
                                        @else
                                            <img src="{{ asset('assets/img/before')}}/{{ $after }}" width="120"/>
                                        @endif
                                    @error('newafter')<p class="text-danger">{{ $message }}</p>@enderror
                                </div>
                        </div> <!-- end col -->
                            <input type="submit" id="SubmitBtn" value="Update Before" name="submit" wire:click.prevent="updateBefore()" class="btn btn-info rounded-pill waves-effect waves-light col-md-2">
                    </form>
                    </div>
                    <!-- end row-->
